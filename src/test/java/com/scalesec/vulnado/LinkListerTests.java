...
public class LinkListerTests {
    ...
    public void getLinks_ValidUrl_ShouldReturnLinks() throws IOException {
        ...
        // Mock Jsoup.connect(url).get()
        Jsoup jsoupMock = mock(Jsoup.class);
        Mockito.when(Jsoup.connect("http://test.com").get()).thenReturn(mockDocument);
    ...
    public void getLinks_InvalidUrl_ShouldThrowIOException() throws IOException {
        LinkLister.getLinks("invalid-url");
    }
    ...
    @Test
    public void getLinksV2_ValidUrl_ShouldNotThrowException() {
        try {
            LinkLister.getLinksV2("http://example.com");
        } catch (BadRequest e) {
            fail("Should not throw BadRequest exception for valid URL");
        }
    }
}
